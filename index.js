...
function phone(strng, num) {
    const currentStr = strng.split(`\n`).map(el => {
        if(el.match(new RegExp(`${num}`, `i`))) {
            return el;
        }
    }).filter(Boolean);
    ...
    if(currentStr.length === 1) {
        let a = currentStr[0].match(RegExp(`<(.+)>`, 'i'));
        let b = currentStr[0].match(RegExp(`(${num})`, 'gi'));
        let c = currentStr[0].replace(RegExp(`${a[0]}`), "").replace(RegExp(`\\+[0-9]{1,2}(\\-[0-9]{3}){2}\\-[0-9]{4}`), "").replace(/[\W\D\S\;\$]/i, "").match(/([^\$\;\:\*\_\?\/\,]+)/gi).map(el => el.replace(/\s{2,}/, ' ').trim()).filter(Boolean).join(' ');

        return `Phone => ${b[0]}, Name => ${a[1]}, Address => ${c}`;
    }
}
